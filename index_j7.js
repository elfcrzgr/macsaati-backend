...
const sportUpdateStatus = {
    football: { lastFullUpdate: 0, lastQuickUpdate: 0, nextMatchTime: null, hasLiveMatch: false, isInQuickMode: false },
    basketball: { lastFullUpdate: 0, lastQuickUpdate: 0, nextMatchTime: null, hasLiveMatch: false, isInQuickMode: false },
    tennis: { lastFullUpdate: 0, lastQuickUpdate: 0, nextMatchTime: null, hasLiveMatch: false, isInQuickMode: false },
    f1: { lastFullUpdate: 0 }
};
            ...
            if (m.spor && toTR(m.spor) === toTR(sportCategory)) {
                ...
                const getCleanWords = (str) => {
                    return str.replace(/İ/g, 'i').replace(/I/g, 'i').replace(/Ğ/g, 'g').replace(/ğ/g, 'g')
                        ...
                        .split(' ').map(w => w.trim()).filter(w => w.length >= 3);
                };
...
// =========================================================================
// 🛠️ YARDIMCI FONKSİYONLAR VE GÜNCELLENEN FETCH ALTYAPISI
// =========================================================================
...
async function fetchData(url) {
    try {
        // HIZLANDIRILDI: Tarama hızını artırmak için bekleme süresini 200-800ms'ye indirdik
...
// EKSİK OLAN TARİH FONKSİYONU GERİ EKLENDİ
...
function hasTodayMatches(cache) {
    ...
    for (const match of cache.values()) {
        if (match.fixedDate === today) return true;
    }
    return false;
}
...
async function updateFootball(targetDates = [getTRDate(0)], isQuickScan = false) {
    ...
    for (const [id, state] of previousMatchStates.entries()) {
        if (state.date && !validDates.includes(state.date) && state.status !== 'inprogress') {
    ...
    // 🔥 AKILLI TARAMA: Sadece cache'de o gün maçı olduğu bilinen ligleri seç
    if (isQuickScan && globalFootballCache.size > 0) {
        ...
        if (activeLeagues.size > 0) {
            ...
            console.log(`⚡ [AKILLI TARAMA] Boş ligler atlanıyor. Sadece aktif ${leaguesToFetch.length} lig sorgulanacak.`);
        }
    }
    ...
    if (successfulDates.length === 0) {
        console.log("⚠️ Yeni veri bulunamadı. Mevcut futbol önbelleği korunuyor.");
        return { hasLiveMatch: sportUpdateStatus.football.hasLiveMatch, nextMatchTimestamp: sportUpdateStatus.football.nextMatchTime, hasAnyMatches: globalFootballCache.size > 0 };
    }
    ...
    if(!isQuickScan) logMatchesBySport({ futbol: futbolMatchesLog });
    ...
    console.log(`  ✅ İşlem bitti. ${hasLiveMatch ? '(🟢 CANLI MAÇ VAR)' : '(⚪ Canlı maç yok)'}`);
    return { hasLiveMatch, nextMatchTimestamp, hasAnyMatches: matches.length > 0 };
}

// =========================================================================
// 🏀 BASKETBOL GÜNCELLEME (AKILLI TARAMA)
// =========================================================================
...
async function updateBasketball(targetDates = [getTRDate(0)], isQuickScan = false) {
    console.log(`🏀 Basketbol: (Mod: ${isQuickScan ? '🚀 HIZLI (Akıllı Tarama)' : '🐢 DETAYLI'})`);
    const validDates = [getTRDate(-2), getTRDate(-1), getTRDate(0), getTRDate(1), getTRDate(2), getTRDate(3)];

    let allEvents = [];
    let successfulDates = [];
    let leaguesToFetch = targetBaskIds;

    // 🔥 AKILLI TARAMA
    if (isQuickScan && globalBasketballCache.size > 0) {
        const activeLeagues = new Set();
...
async function updateTennis(targetDates = [getTRDate(0)]) {
    ...
    if (successfulDates.length === 0) {
        return { hasLiveMatch: sportUpdateStatus.tennis.hasLiveMatch, nextMatchTimestamp: sportUpdateStatus.tennis.nextMatchTime, hasAnyMatches: globalTennisCache.size > 0 };
    }
...
const CIRCUIT_DETAILS = {
    "bahrain": { laps: "57", length: "5.412 km", record: "1:31.447 - Pedro de la Rosa" }, "jeddah": { laps: "50", length: "6.174 km", record: "1:30.734 - Lewis Hamilton" },
    "albert_park": { laps: "58", length: "5.278 km", record: "1:19.813 - Charles Leclerc" }, "suzuka": { laps: "53", length: "5.807 km", record: "1:30.983 - Lewis Hamilton" },
    "shanghai": { laps: "56", length: "5.451 km", record: "1:32.238 - Michael Schumacher" }, "miami": { laps: "57", length: "5.412 km", record: "1:29.708 - Max Verstappen" },
    "imola": { laps: "63", length: "4.909 km", record: "1:15.484 - Lewis Hamilton" }, "monaco": { laps: "78", length: "3.337 km", record: "1:12.909 - Lewis Hamilton" },
    "villeneuve": { laps: "70", length: "4.361 km", record: "1:13.078 - Valtteri Bottas" }, "catalunya": { laps: "66", length: "4.675 km", record: "1:18.149 - Max Verstappen" },
    "red_bull_ring": { laps: "71", length: "4.318 km", record: "1:05.619 - Carlos Sainz" }, "silverstone": { laps: "52", length: "5.891 km", record: "1:27.097 - Max Verstappen" },
    "hungaroring": { laps: "70", length: "4.381 km", record: "1:16.627 - Lewis Hamilton" }, "spa": { laps: "44", length: "7.004 km", record: "1:46.286 - Valtteri Bottas" },
    "zandvoort": { laps: "72", length: "4.259 km", record: "1:11.097 - Lewis Hamilton" }, "monza": { laps: "53", length: "5.793 km", record: "1:21.046 - Rubens Barrichello" },
    "baku": { laps: "51", length: "6.003 km", record: "1:43.009 - Charles Leclerc" }, "marina_bay": { laps: "62", length: "4.940 km", record: "1:35.867 - Lewis Hamilton" },
    "americas": { laps: "56", length: "5.513 km", record: "1:36.169 - Charles Leclerc" }, "rodriguez": { laps: "71", length: "4.304 km", record: "1:17.774 - Valtteri Bottas" },
    "interlagos": { laps: "71", length: "4.309 km", record: "1:10.540 - Valtteri Bottas" }, "vegas": { laps: "50", length: "6.201 km", record: "1:35.490 - Oscar Piastri" },
    "losail": { laps: "57", length: "5.419 km", record: "1:24.319 - Max Verstappen" }, "yas_marina": { laps: "58", length: "5.281 km", record: "1:26.103 - Max Verstappen" }
};
...
async function updateF1() {
    try {
        ...
        const countryToCode = {
            "Bahrain": "bh", "Saudi Arabia": "sa", "Australia": "au", "Japan": "jp", "China": "cn", "USA": "us", "United States": "us", "Italy": "it",
            "Monaco": "mc", "Canada": "ca", "Spain": "es", "Austria": "at", "UK": "gb", "Hungary": "hu", "Belgium": "be", "Netherlands": "nl",
            "Azerbaijan": "az", "Singapore": "sg", "Mexico": "mx", "Brazil": "br", "Qatar": "qa", "UAE": "ae"
        };

        races.forEach(race => {
            const circuitId = race.Circuit.circuitId; const countryName = race.Circuit.Location.country;
            ...
            const addSession = (sessionName, dateStr, timeStr) => {
                ...
                finalEvents.push({
                    id: `${race.round}_${sessionName.replace(/\s/g, '')}`, fixedDate: `${dayAndMonth} ${dayName}`, fixedTime: dateObj.toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' }),
                    timestamp: dateObj.getTime(), broadcaster: "beIN Sports / F1 TV", grandPrix: race.raceName, sessionName: sessionName, trackName: race.Circuit.circuitName,
                    circuitStats: { laps: stats.laps, length: stats.length, record: stats.record }, countryLogo: F1_LOGO_BASE + flagCode + ".png", tournamentLogo: F1_TOURNAMENT_BASE + circuitId + ".png"
                });
            };
...
// =========================================================================
async function main() {
    if (!apnProvider) { console.error("⚠️ KRİTİK HATA: APNs Sağlayıcı başlatılamadı! Lütfen .p8 dosyasını ve Team ID'yi kontrol et."); return; }
    ...
    while (true) {
        try {
            ...
            if (lastPeriodicUpdate < activeTarget) {
                console.log("🔄 [PERİYODİK GÜNCELLEME] Ana Saat Dilimi Tetiklendi! (Detaylı Tarama)");
                const days4 = [getTRDate(-1), getTRDate(0), getTRDate(1), getTRDate(2)];

                // 🔥 "false" parametresi = Detaylı tarama (Tüm ligleri gezer)
                const footballResult = await updateFootball(days4, false);
                const basketballResult = await updateBasketball(days4, false);
                const tennisResult = await updateTennis(days4);
                await updateF1();

                sportUpdateStatus.football.nextMatchTime = footballResult.nextMatchTimestamp; sportUpdateStatus.basketball.nextMatchTime = basketballResult.nextMatchTimestamp;
                sportUpdateStatus.tennis.nextMatchTime = tennisResult.nextMatchTimestamp; sportUpdateStatus.football.hasLiveMatch = footballResult.hasLiveMatch;
            ...
            if (sportUpdateStatus.football.hasLiveMatch) {
                if (now - sportUpdateStatus.football.lastQuickUpdate >= ONE_MIN_MS) {
                    console.log("⚽ [HIZLI DÖNGÜ] Canlı futbol maçı var!");
                    // 🔥 "true" parametresi = Akıllı tarama (Sadece aktif ligler)
                    const footResult = await updateFootball(todayOnly, true); 
                    sportUpdateStatus.football.lastQuickUpdate = now; sportUpdateStatus.football.hasLiveMatch = footResult.hasLiveMatch; sportUpdateStatus.football.nextMatchTime = footResult.nextMatchTimestamp;
                }
            } else if (sportUpdateStatus.football.nextMatchTime && now >= (sportUpdateStatus.football.nextMatchTime - ONE_MIN_MS * 1.1)) {
                if (now - sportUpdateStatus.football.lastQuickUpdate >= ONE_MIN_MS) {
                    console.log("⏰ [FUTBOL YAKLAŞAN] Yaklaşan maç saati yaklaştı!");
                    const footResult = await updateFootball(quickScanDates, true); 
                    sportUpdateStatus.football.lastQuickUpdate = now; sportUpdateStatus.football.hasLiveMatch = footResult.hasLiveMatch; sportUpdateStatus.football.nextMatchTime = footResult.nextMatchTimestamp;
                }
            }

            const hasUpcomingBasketball = sportUpdateStatus.basketball.nextMatchTime && now >= (sportUpdateStatus.basketball.nextMatchTime - ONE_MIN_MS * 11);
            if ((sportUpdateStatus.basketball.hasLiveMatch || hasUpcomingBasketball) && now - sportUpdateStatus.basketball.lastQuickUpdate >= TEN_MIN_MS) {
                ...
                const basketResult = await updateBasketball(quickScanDates, true);
                sportUpdateStatus.basketball.lastQuickUpdate = now; sportUpdateStatus.basketball.nextMatchTime = basketResult.nextMatchTimestamp;
            }

            const hasUpcomingTennis = sportUpdateStatus.tennis.nextMatchTime && now >= (sportUpdateStatus.tennis.nextMatchTime - ONE_MIN_MS * 11);
            if ((sportUpdateStatus.tennis.hasLiveMatch || hasUpcomingTennis) && now - sportUpdateStatus.tennis.lastQuickUpdate >= TEN_MIN_MS) {
                ...
                sportUpdateStatus.tennis.lastQuickUpdate = now; sportUpdateStatus.tennis.nextMatchTime = tennisResult.nextMatchTimestamp;
            }

            let sleepTime = TEN_MIN_MS;
            const isFootballActive = sportUpdateStatus.football.hasLiveMatch || (sportUpdateStatus.football.nextMatchTime && now >= (sportUpdateStatus.football.nextMatchTime - ONE_MIN_MS * 12));

            if (isFootballActive) {
                sleepTime = ONE_MIN_MS;
                console.log("⚡ Aktif maç takibi, 1 dakika sonra kontrol...");
                ...
                let timeToNextBask = TEN_MIN_MS; let timeToNextTen = TEN_MIN_MS;
                if (sportUpdateStatus.basketball.hasLiveMatch || hasUpcomingBasketball) timeToNextBask = TEN_MIN_MS - (now - sportUpdateStatus.basketball.lastQuickUpdate);
                if (sportUpdateStatus.tennis.hasLiveMatch || hasUpcomingTennis) timeToNextTen = TEN_MIN_MS - (now - sportUpdateStatus.tennis.lastQuickUpdate);
        ...
        } catch (e) { console.error("🚨 Hata:", e.message); await new Promise(r => setTimeout(r, ONE_MIN_MS)); }
    }
}
main();