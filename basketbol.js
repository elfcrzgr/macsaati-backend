...
async function fetchApiSports(url) {
    try {
        ...
        if (response.data && response.data.response) return response.data.response; return [];
    } catch (e) { return null; }
}
...
async function updateBasketball(targetDates) {
    ...
    for (const date of targetDates) {
        ...
        if (games !== null && games.length > 0) allGames.push(...games);
        await new Promise(r => setTimeout(r, 1000));
    }

    if (allGames.length === 0) return { hasLiveMatch: sportUpdateStatus.hasLiveMatch, nextMatchTimestamp: sportUpdateStatus.nextMatchTime }; 
