// =========================================================================
// 🔥 SOFASCORE TLS BYPASS - CURL_CFFI ENTEGRASYON
// =========================================================================
// Bu, önceki kodunuzun başında yer alacak
...
// =========================================================================
// 🔥 BAŞLATMA - ESKI FETCH YERINE YENİ BYPASS CLIENT'I KULLAN
// =========================================================================

const tlsClient = new PythonTLSBypassClient();

// Eski fetchData fonksiyonunu BUNU ile değiştir:
        ...
        if (match.status === 'inprogress' && prev.status !== null) {
            if (prev.homeScore !== currH || prev.awayScore !== currA) {
                ...
                if (isGoal) {
                    ...
                    try {
                        const incidentsData = await fetchData(`https://api.sofascore.com/api/v1/event/${match.id}/incidents`);
...
async function updateFootball(targetDates = [getTRDate(0)]) {
    ...
    for (const date of targetDates) {
        const data = await fetchData(`https://api.sofascore.com/api/v1/sport/football/scheduled-events/${date}?_=${Date.now()}`);
        if (data?.events) {
...
async function main() {
    ...
    loadState();
    console.log("============================================================");
    console.log("🟢 J7 CANLI SUNUCU BAŞLADI (PYTHON TLS BYPASS SISTEMI İLE)");
    console.log("============================================================");
