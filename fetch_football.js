...
async function start() {
    console.log("🚀 Futbol motoru başlatılıyor (events/all taraması)...");
    ...
    for (const id of stubbornLeagueIds) {
        try {
            ...
            if (seasonsData && seasonsData.seasons && seasonsData.seasons.length > 0) {
                ...
                // --- ADIM 3A: events/all/* ile tüm sayfaları tara ---
                let allPagesMatches = 0;
                for (let pageNum = 0; pageNum <= 10; pageNum++) {
                    try {
                        await page.goto(`https://api.sofascore.com/api/v1/unique-tournament/${id}/season/${seasonId}/events/all/${pageNum}`, { waitUntil: 'networkidle2' });
                        ...
                        if (eventsData && eventsData.events && eventsData.events.length > 0) {
                            ...
                            if (targetEvents.length > 0) {
                                console.log(`   ✅ events/all/${pageNum}: ${targetEvents.length} maç`);
                                allPagesMatches += targetEvents.length;
                                allEvents = allEvents.concat(targetEvents);
                            }

                            // Eğer hasNextPage false ise dur
                            if (!eventsData.hasNextPage) {
                                console.log(`   ℹ️ Son sayfaya ulaşıldı (pageNum: ${pageNum})`);
                                break;
                            }
                        } else {
                            console.log(`   ℹ️ events/all/${pageNum}: boş, tarama durdu`);
                            break;
                        }
                    } catch (e) {
                        break;
                ...
                if (allPagesMatches === 0) {
                    console.log(`   ℹ️ Turnuva ${id} için bugün maç bulunamadı`);
                }
            }
