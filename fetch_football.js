...
async function start() {
    console.log("🚀 Futbol motoru başlatılıyor (+ Stage/Playoff Taraması)...");
    ...
    for (const date of validDates) {
        try {
            ...
            if (data && data.events) {
                ...
                console.log(`   ✅ Genel API: ${filtered.length} maç bulundu`);
    ...
    try {
        ...
        if (liveData && liveData.events) {
            ...
            console.log(`   ✅ Canlı Maçlar: ${filteredLive.length} maç bulundu`);
            allEvents = allEvents.concat(filteredLive);
        }
    } catch (e) { console.error(`❌ Canlı Maç Hatası:`, e.message); }

    // --- 3. ADIM: DERİN DALIŞ (İNATÇI LİGLER) ---
    for (const id of stubbornLeagueIds) {
        try {
            ...
            if (seasonsData && seasonsData.seasons && seasonsData.seasons.length > 0) {
                ...
                // --- 3A: STAGE/PLAYOFF TARAMASI ---
                try {
                    console.log(`   📍 Sezon ID: ${seasonId}`);
                    console.log(`   🎯 Stage'ler taranıyor...`);
                    
                    await page.goto(`https://api.sofascore.com/api/v1/unique-tournament/${id}/season/${seasonId}/stages`, { waitUntil: 'networkidle2' });
                    const stagesData = await page.evaluate(() => { try { return JSON.parse(document.body.innerText); } catch(e) { return null; } });
                    
                    if (stagesData && stagesData.stages && stagesData.stages.length > 0) {
                        console.log(`      ✅ ${stagesData.stages.length} stage bulundu`);
                        
                        // Her stage için maçları çek
                        for (const stage of stagesData.stages) {
                            ...
                            for (let pageNum = 0; pageNum <= 3; pageNum++) {
                                try {
                                    await page.goto(
                                        `https://api.sofascore.com/api/v1/unique-tournament/${id}/season/${seasonId}/stage/${stage.id}/events/all/${pageNum}`,
                                    ...
                                    if (eventsData && eventsData.events && eventsData.events.length > 0) {
                                        ...
                                        if (targetEvents.length > 0) {
                                            console.log(`         ✅ ${stageName} (Sayfa ${pageNum}): ${targetEvents.length} maç`);
                                            allEvents = allEvents.concat(targetEvents);
                                        }
                                    } else {
                                        break; // Boş sayfa = daha yok
                                    }
                                } catch (e) {
                                    break;
                                }
                    ...
                    console.log(`      ℹ️ Stage taraması başarısız, genel tarama yapılıyor...`);
                }

                // --- 3B: GENEL TARAMA (FALLBACK) ---
                for (const pageType of ['next/0', 'next/1', 'next/2', 'last/0', 'last/1', 'last/2']) {
                    try {
                        ...
                        if (eventsData && eventsData.events) {
                            ...
                            if (targetEvents.length > 0) {
                                console.log(`      ✅ Genel Tarama (${pageType}): ${targetEvents.length} maç`);
                                allEvents = allEvents.concat(targetEvents);
                            }
                        }
                    } catch (e) { }
