...
async function start() {
    console.log("🚀 Maç Saati Motoru: Akıllı Keşif ve 27 Mart taraması başladı...");
    ...
    // --- 1. ADIM: GENEL API TARAMASI + OTOMATİK KEŞİF ---
        ...
        try {
            ...
            if (data && data.events) {
                const filtered = data.events.filter(e => {
                    ...
                    // KURAL: Ya senin listende olsun ya da SofaScore'un "popüler" (priority > 100) dediği maç olsun
    ...
    // --- 3. ADIM: AYIKLAMA VE YAYINCI ATAMA ---
    ...
    for (const e of allEvents) {
        ...
        const matchObj = {
            ...
            // --- BURASI KRİTİK ---
            // Listende varsa yayıncıyı yaz, yoksa "Belirlenmedi" de (Kullanıcıyı yanıltma)
    ...
    console.log(`\n✅ İşlem Tamamlandı. Toplam ${finalMatches.length} maç (Keşfedilenler dahil) kaydedildi.`);
    await browser.close();
}
