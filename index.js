...
const { exec } = require('child_process'); // 🛠️ YENİ EKLENDİ: Git push otomasyonu için
...
const getTennisBroadcaster = (tourName) => {
    ...
    if (name.includes("US OPEN")) return "Eurosport";
    if (name.includes("AUSTRALIAN OPEN")) return "Eurosport";
    if (name.includes("ROLAND GARROS") || name.includes("FRENCH OPEN")) return "Eurosport";
...
// =========================================================================
// 🚀 ANA ORKESTRATÖR (Sırayla tüm sporları çalıştırır)
// =========================================================================
async function start() {
    console.log("🚀 MAÇ SAATİ MASTER MOTORU BAŞLATILDI...");
    ...
    try {
        ...
        console.error("❌ ANA MOTOR HATASI:", e);
    } finally {
        await browser.close();
        console.log("\n✅ TÜM İŞLEMLER BAŞARIYLA TAMAMLANDI VE TARAYICI KAPATILDI.");
    }
}
...
async function runFootball(page) {
    ...
    for (const e of allEvents) {
        ...
        if (isInProgress) timeString += " CANLI"; 
        ...
        // 📊 Özete Ekle
        if (ELITE_FOOT_IDS.includes(utId) && !isExcludedCategory) {
            addToSummary("football", utName);
        } else if (ALL_FOOT_TARGETS.includes(utId)) {
            addToSummary("football", utName);
        }
        ...
        finalMatchesMap.set(matchKey, {
            ...
            broadcaster: getFootBroadcaster(utId, hName, aName, tName, utName), 
...
async function runBasketball(page) {
    ...
    } catch (e) { console.log("⚠️ Basketbol ana sayfa ısınma atlandı."); }
    ...
    for (const date of [getTRDate(-1), trToday, trTomorrow]) {
        try {
            ...
            if (data && data.events) {
                const filtered = data.events.filter(e => targetBaskIds.includes(e.tournament?.uniqueTournament?.id));
...
async function runTennis(page) {
    ...
    try {
        await page.goto('https://www.sofascore.com', { waitUntil: 'domcontentloaded', timeout: 60000 });
        await new Promise(r => setTimeout(r, 5000)); 
    } catch (e) { console.log("⚠️ Tenis ana sayfa ısınma atlandı."); }
...
// =========================================================================
// 🚀 CANLI SKOR DÖNGÜSÜ (OTOMATİK GİT PUSH İLE)
// =========================================================================
async function loop() {
    console.log("🟢 CANLI SKOR MOTORU AKTİF (Sonsuz Döngü)");
    
    while (true) {
        const simdi = new Date().toLocaleTimeString('tr-TR', { timeZone: 'Europe/Istanbul' });
        console.log(`\n[${simdi}] 🔄 Tarama Başlatıldı...`);
        
        try {
            // Ana fonksiyonu çalıştır (Bütün JSON'ları günceller)
            await start(); 
            console.log(`[${simdi}] ✅ JSON'lar oluşturuldu, GitHub'a gönderiliyor...`);
            
            // Terminalde otomatik Git Push komutu çalıştırıyoruz
            exec('git add . && git commit -m "Canlı Skor Güncellemesi" && git push', (error, stdout, stderr) => {
                if (error) {
                    // Git, "commit edilecek bir değişiklik yok" dediğinde hata fırlatır, bunu süzüyoruz:
                    if (error.message.includes("nothing to commit") || error.message.includes("clean")) {
                        console.log(`☁️ JSON'larda değişiklik yok, push atlandı.`);
                    } else {
                        console.error(`❌ Git Push Hatası: ${error.message}`);
                    }
                } else {
                    console.log(`☁️ GitHub'a başarıyla yüklendi!`);
                }
            });

        } catch (e) {
            console.error(`[${simdi}] ❌ Motor Çöktü/Hata verdi:`, e.message);
        }

        console.log("⏱️ Yeni tarama için 60 saniye bekleniyor...");
        await new Promise(r => setTimeout(r, 60000)); // Sistemi yormamak için 1 dakika mola
    }
}
